...
function score5(cards) {
    ...
    const handRank = (() => {
        if (isStraight && isFlush)
            return 8;
        if (groupsSorted[0][1] === 4)
            return 7;
        if (groupsSorted[0][1] === 3 && groupsSorted[1]?.[1] === 2)
            return 6;
        if (isFlush)
            return 5;
        if (isStraight)
            return 4;
        if (groupsSorted[0][1] === 3)
            return 3;
        if (groupsSorted[0][1] === 2 && groupsSorted[1]?.[1] === 2)
            return 2;
        if (groupsSorted[0][1] === 2)
            return 1;
        return 0;
    })();
...
export function evaluatePlayerHand(holeCards, communityCards, dropZone) {
